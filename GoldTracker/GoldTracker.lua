-- アドオン情報設定
_addon.name     = 'GoldTracker'
_addon.author   = 'dsw'
_addon.version  = '2020-05-21'
_addon.language = 'japanese'
_addon.command  = 'goldtracker'
_addon.commands = { 'gt' }
...
local addon =
{
	...
	UpdateByItemPacket = function( this, packetData )
		...
		return ( p.Item == GIL_ITEM_ID and p.Count >= 0 )
	end,
	...
	Refresh = function( this )
		if( this.isDirty == false ) then return end
		this.isDirty = false
		
		UI:Set( windower.ffxi.get_items( 'gil' ) )
	end,
...
addon.RegisterEvents = function( this )
	...
	windower.register_event( 'incoming chunk', function( id, original, modified, isInjected, isBlocked )
		...
		elseif( id == TREASURE_FIND_ITEM_PACKET ) then
			-- お宝を発見
			if( this:UpdateByTreasurePacket( original ) == true ) then
				this.isDirty = true
			end
