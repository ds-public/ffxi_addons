-- アドオン情報設定
_addon.name     = 'HealthGauge'
_addon.author   = 'dsw'
_addon.version  = '2020-05-22'
_addon.language = 'japanese'
_addon.command  = 'healthgauge'
_addon.commands = { 'hg' }
...
local HEALING_START_ID          = 33
local HEALING_END_ID            =  0
...
local addon =
{
	...
	Refresh = function( this )
		...
		local hpValue = player.vitals.hp
		local hpRatio = player.vitals.hpp
		local mpValue = player.vitals.mp
		local mpRatio = player.vitals.mpp
		local tpValue = player.vitals.tp

		UI:Set( hpValue, hpRatio, mpValue, mpRatio, tpValue, false )
		this.isDirty = false
...
addon.RegisterEvents = function( this )
	...
	windower.register_event( 'prerender',  function( ... )
		...
		if( this.isAwake == false ) then
			this.isAwake  = true
			UI:Set( 0, 0, 0, 0, 0, true )	-- 最初は0状態にしておく
			UI:FadeIn()
		end

		if( this.isDirty == true ) then this:Refresh() end

		-- 値が取れるまで再取得を繰り返す
		if( this.isReady == false ) then
